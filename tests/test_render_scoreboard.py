...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for expected in (
            ...
            "| synthetic | `treewidth --treewidth-order min-fill` | 1 / 1 | 1.2 us | cache hits=1, misses=2, hit rate=0.333; cache trace lookup=2 events/2.0 us, store=1 events/3.0 us; tw width 2; max table 16; 32 join pairs; branch dispatch splits=3/5.0 us max components=2, tw delegates=1/7.0 us max vars=33, rw delegates=1/11.0 us max vars=40; branch policy fallthroughs=0, tw skips=2, rw skips=7; tw skip reasons width=2; rw skip reasons table-forecast=3, join-pair-forecast=4; branch table forecast rw=2, tw=128; branch join forecast rw=9, tw=64 |",
            "`qiskit-statevector`",
