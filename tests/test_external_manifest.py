...
def main() -> int:
    ...
    check_angle_folding(load_manifest_module(builder))
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # End-to-end CLI check for the r(theta,phi)-style macro arithmetic folding: previously
        # this expanded to "u(pi/4, -pi/2 + pi/2, pi/2 - pi/2)", which qasm2sop's tokenizer
        # mis-split on the internal whitespace (see check_angle_folding above for the unit-level
        # check of the same folding logic). Uses the exact/dyadic variant here (theta=pi/4) so the
        # manifest builder's default exact-mode import actually succeeds end-to-end; the
        # continuous-theta variant is exercised directly via inline_simple_gates() above.
        macro_arith_root = root / "macro_arith"
        macro_arith_root.mkdir()
        (macro_arith_root / "macro_arith.qasm").write_text(
            MACRO_ARITHMETIC_EXACT_QASM_SAMPLE, encoding="utf-8"
        )
        macro_arith_cases, macro_arith_stderr = run_manifest(
            builder,
            qasm2sop,
            str(macro_arith_root),
            "--source-prefix",
            "smoke",
            "--inline-simple-gates",
        )
        if len(macro_arith_cases) != 1:
            raise AssertionError(
                f"unexpected macro arithmetic manifest:\n{macro_arith_cases}\n{macro_arith_stderr}"
            )
        macro_arith_qasm = "\n".join(macro_arith_cases[0]["qasm_lines"])
        if "u(pi/4,0,0) q[0];" not in macro_arith_qasm:
            raise AssertionError(f"macro arithmetic was not folded end-to-end:\n{macro_arith_qasm}")
