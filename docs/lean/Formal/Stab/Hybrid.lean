...
vertex the algorithm may pick one of three moves:

* `Move.join` — multiply the children's lists termwise, `Λ_u ≤ Λ_L · Λ_R`;
* `Move.rebuild` — throw the children away and re-derive the table from the instance, giving
  `Λ_u ≤ sr(τ_u)` decompositions of the magic part below `u`;
* `Move.point` — a join restricted to *point* tables, where equal supports are merged by
...
* Only the operation count is modelled, exactly as in `Formal.Core.Cost`: `2` per leaf and
  `Λ_L · Λ_R` per internal vertex. Soundness of the three moves — that the list really does sum
...
/-- The three moves available at an internal vertex of `alg:stabjoin`: termwise `join` of the
...
the move strategy `S` and the abstract stabilizer-rank function `sr`. A leaf emits the two
point tables `x_v = 0, 1`; an internal vertex pays according to its move. -/
noncomputable def Lam (I : SopInstance) (sr : RTree I.V → ℕ) (S : RTree I.V → Move) :
    RTree I.V → ℕ
  | .leaf _ => 2
  | .node L R =>
...
variable (I : SopInstance) (sr : RTree I.V → ℕ) (S : RTree I.V → Move)
...
magic part below `u`. -/
theorem Lam_node_rebuild {L R : RTree I.V} (h : S (.node L R) = Move.rebuild) :
    Lam I sr S (.node L R) = sr (.node L R) := by
  rw [Lam, h]

/-- The `Join` annotation of `alg:stabjoin`: a termwise product of the children's lists,
`Λ_u ≤ Λ_L · Λ_R`. -/
theorem Lam_node_join {L R : RTree I.V} (h : S (.node L R) = Move.join) :
...
theorem Lam_node_point {L R : RTree I.V} (h : S (.node L R) = Move.point) :
    Lam I sr S (.node L R)
      = min (Lam I sr S L * Lam I sr S R) (I.statesSig (.node L R)) := by
  rw [Lam, h]

/-- The `2^{ρ_u}` half of the `Point` move: after hash merging at most one entry survives per
occupied boundary signature. -/
theorem Lam_node_point_le {L R : RTree I.V} (h : S (.node L R) = Move.point) :
...
/-- `PointForm S u`: every internal vertex of the subtree `u` takes the `Point` move. This is
...
def PointForm (I : SopInstance) (S : RTree I.V → Move) : RTree I.V → Prop
  | .leaf _ => True
  | .node L R => S (.node L R) = Move.point ∧ PointForm I S L ∧ PointForm I S R

@[simp] theorem PointForm_leaf (v : I.V) : PointForm I S (.leaf v) := trivial

/-- Point form at a node is the move at the node together with point form at both children. -/
theorem PointForm_node {L R : RTree I.V} :
    PointForm I S (.node L R)
      ↔ S (.node L R) = Move.point ∧ PointForm I S L ∧ PointForm I S R := Iff.rfl
...
noncomputable def costHybrid (I : SopInstance) (sr : RTree I.V → ℕ) (S : RTree I.V → Move) :
    RTree I.V → ℕ
  | .leaf _ => 2
...
theorem Lam_le_of_clifford (hsr : ∀ u, sr u ≤ 1) (hS : ∀ u, S u = Move.rebuild) :
