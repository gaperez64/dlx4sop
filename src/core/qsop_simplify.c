 ...
 * duplicates in place. simplify_round assumes this shape on entry so that the degree counts it
...
static bool is_zero_witness(const qsop_instance_t *inst, uint64_t sign_coeff) {
  ...
  *changed = false;
  ...
  for (uint32_t x = 0; x < n; x++) {
    if (degree[x] == 0U && inst->unary[x] == sign_coeff) {
      free(degree);
      if (is_zero_witness(inst, sign_coeff)) {
        return true; /* already the witness: fixpoint */
      }
      if (!collapse_to_zero(inst, sign_coeff)) {
        return false;
      }
      *changed = true;
      return true;
